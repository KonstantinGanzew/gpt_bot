...
async def help_key(call: CallbackQuery, callback_data: dict):
    ...
    await call.message.edit_text(
        'Я бот OpenAi,\nсоздан @kureed для облегчения жизни, так как всякие ресурсы OpenAi либо имеют бешаные цены, '
        'либо работают 1 день, было решено взять свой токен и попробовать общение через него.\nЧтобы продолжить '
